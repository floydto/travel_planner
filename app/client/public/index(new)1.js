window.onload = async function () {
	...
	if (login_json.result == 'ok') {
		...
		document.querySelector('#itineraryMgt').classList.remove('hide');
		document.querySelector('#logoutBtn').classList.remove('hide');
...
async function loadHotJourney() {
	...
	console.log(route)
	console.log(route.img_path);
	let imagepath = `"img/Temp.jpg"`

	if (route.img_path != null) {
		imagepath = `"${route.img_path}"`
	}
	if (i === 0) {
	container.innerHTML = `
		<div>
                <div class="image fit">
				  <a href="#"><img src=${imagepath};
				  alt="Card image cap" />
				  </a>  
				  <p class="titleContain">${route.title}</p>
...
async function loadTrendingPlaces() {
	...
	console.log(places)
	let container = document.querySelector(".grid-style");
	let i = 0; 
	for(route of places) {
